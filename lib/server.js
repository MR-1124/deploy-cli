...
// SPA fallback: a missing *route* (extension-less path like /about or /team/contact)
// serves the app shell instead of 404ing, so React Router deep links work. Asset
// requests (app.js, logo.png, /_next/...) must keep returning a real 404 — the HTML
// shell is never served in place of a missing file.
function looksLikeRoute(rel) {
  const last = rel.split("/").pop() || "";
  if (!last) return true; // trailing slash
  return !path.extname(last) && !last.startsWith(".");
}

function serveStatic(res, filePath, fallbackIndex = null) {
  let target = filePath;
  try {
    const stat = fs.statSync(target);
    if (stat.isDirectory()) {
      const idx = path.join(target, "index.html");
      if (!fs.existsSync(idx)) throw new Error("no index");
      target = idx;
    }
    ...
    if (fallbackIndex && fs.existsSync(fallbackIndex)) {
      return serveStatic(res, fallbackIndex, null);
    }
  ...
  return http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (parts[0] === "api" && parts[1] === "deploy" && req.method === "POST") {
        ...
        if (idemKey) {
          registry.idempotency[idemKey] = response;
          const keys = Object.keys(registry.idempotency);
          if (keys.length > 100) {
            for (const oldKey of keys.slice(0, keys.length - 100)) {
              delete registry.idempotency[oldKey];
            }
          }
        }
      ...
      if (req.method === "GET" || req.method === "HEAD") {
        ...
        let filePath;
        try {
          filePath = safeJoin(deployDir, rel);
        } catch {
        ...
        const spaFallback =
          req.method === "GET" && rel && looksLikeRoute(rel)
            ? path.join(deployDir, "index.html")
            : null;
        return serveStatic(res, filePath, spaFallback);
      }
