...
function splitUstarPath(fullPath) {
  ...
  for (let i = fullPath.length - 1; i >= 0; i--) {
    if (fullPath[i] === "/") {
      ...
      const prefix = fullPath.slice(0, i);
      if (name.length <= 100 && prefix.length <= 155) {
        return { name, prefix };
      }
    }
  }
  // Path exceeds ustar limits — cannot safely represent without PAX headers
  throw new Error(
    `Path too long for ustar tar format (${fullPath.length} chars, max 255): ${fullPath}. ` +
    `Rename to a shorter path and try again.`
  );
}

function ustarHeader(fullPath, { type = "0", size = 0, mode = 0o644, mtime = 0 }) {
  ...
  if (prefix) write(345, prefix, 155); // prefix (for paths > 100 chars)
...
export function extractTar(buf, destDir) {
  ...
  while (off + BLOCK <= buf.length) {
    ...
    const target = safeJoin(destDir, fullName);
    if (type === "5") {
