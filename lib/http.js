...
/**
 * Run asyncFn over items with bounded concurrency.
 * Returns array of results in the original item order.
 */
export async function mapConcurrent(items, limit = 6, asyncFn) {
  if (!items.length) return [];
  const results = new Array(items.length);
  let nextIdx = 0;
  let firstError = null;
  const workers = new Array(Math.min(limit, items.length)).fill(0).map(async () => {
    while (nextIdx < items.length && !firstError) {
      const idx = nextIdx++;
      try {
