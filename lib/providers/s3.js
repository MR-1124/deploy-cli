...
export function sign({ accessKeyId, secretAccessKey, region, bucket, key, method = "PUT", query = {}, payloadHash, date = new Date(), endpoint = null }) {
...
export function signPut(opts) {
  return sign({ ...opts, method: "PUT", query: {}, payloadHash: sha256hex(opts.content) });
}

export async function s3Fetch(creds, { key = "", method, query = {}, body = null }) {
  const endpoint = ENDPOINT();
  const payloadHash = body ? sha256hex(body) : sha256hex("");
  const signed = sign({ ...creds, key, method, query, payloadHash, endpoint });
  const url = endpoint
...
export async function deploy({ project, outDir, branch, preview, flags, config, rc }) {
  ...
  for (const f of pre.files) {
    ...
    await s3Fetch(creds, { key: `${prefix}/${f.rel}`, method: "PUT", body: content });
    done++;
