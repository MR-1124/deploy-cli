...
export async function deploy({ project, outDir, branch, preview, flags, config, rc }) {
  ...
  if (method === "zip") {
    ...
    let done = 0;
    const requiredItems = required.map((sha) => filesBySha.get(sha)).filter(Boolean);
    await mapConcurrent(requiredItems, 6, async (f) => {
      ...
      progress("uploading", done, requiredItems.length);
    });
    if (flags.wait !== false) {
